    ...
    public function mostrarDatos()
    {
        ...
        if (!empty($datos)) {
            foreach ($datos as $producto) {
                ...
                $data['precio_compra'] = number_format((empty($producto['precio'])) ? 0 : $producto['precio'], 0, '.', '');
                $data['precio_venta'] = number_format((empty($producto['precio'])) ? 0 : $producto['precio'], 0, '.', '');
                $data['cantidad'] = $producto['cantidad'];
                $subTotalCompra = $data['precio_compra'] * $producto['cantidad'];
                $subTotalVenta =    $data['precio_venta'] * $producto['cantidad'];
                $data['subTotalCompra'] = number_format($subTotalCompra, 0);
                $data['subTotalVenta'] = number_format($subTotalVenta, 0);
                array_push($array['productos'], $data);
                $totalCompra += $subTotalCompra;
                $totalVenta += $subTotalVenta;
            }
        }
        $array['totalCompra'] = number_format($totalCompra, 0);
        $array['totalVenta'] = number_format($totalVenta, 0);
        $array['totalVentaSD'] = number_format($totalVenta, 0, '.', '');
